...
class DisasterActivity : AppCompatActivity() {
    var MbtnCrime:Button ?= null
    var listusers: ListView?=null
    var Adapter:CustomAdapterTwo ?=null
    var users:ArrayList<UserTwo> ?= null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_disaster
        )
        MbtnCrime=findViewById(R.id.MbtnViewCrime)
        MbtnCrime!!.setOnClickListener {
            var intent=Intent(this,CrimeActivity::class.java)
            startActivity(intent)
        }



        listusers=findViewById(R.id.MlistDisasters)
        users=ArrayList()
        Adapter = CustomAdapterTwo(this, users!!)
        val reference= FirebaseDatabase.getInstance().
        getReference().child("UserTwo")
...
//    User for loop to add the user on the arraylist
                for (snap in  snapshot.children){
                    var user=snap.getValue(UserTwo::class.java)
                    users!!.add(user!!)
                }
...
//        Set on item click listener to the listview
        listusers!!.setOnItemClickListener { adapterView, view, i, l ->
            ...
            getReference().child("UserTwo/$UserId")
